...
const AdminDashboardShell: React.FC = () => {
  ...
  return (
    <div className="admin-dashboard-theme flex min-h-screen flex-col bg-[#f8f9fa] md:flex-row">
...
function AdminDashboardHeader({ userFullName }: { userFullName?: string }) {
  return (
    <motion.header
      ...
      className="sticky top-0 z-30 border-b border-[#dce2ec] bg-[#f8f9fa]/90 px-4 py-4 backdrop-blur sm:px-6 lg:px-8"
    >
  ...
  return (
    <>
      ...
      <nav className="flex-1 space-y-2 overflow-y-auto px-4 py-5">
        ...
        >
          <button
            ...
            className={`relative flex w-full items-center justify-between overflow-hidden rounded-lg px-3 py-3 text-sm font-semibold transition-colors ${
              locationPathname.startsWith("/dashboard/admin/courses")
                ? "bg-primary text-white shadow-sm before:absolute before:inset-y-0 before:left-0 before:w-1.5 before:bg-secondary-container"
      ...
      <div className="border-t border-[#e7ebf1] p-4">
        <div
          className={`${showAccountCard ? "mb-3 block" : "hidden"} rounded-xl bg-on-primary-fixed p-4 text-white`}
        >
...
function AdminSidebarLink({
  ...
  return (
    <button
      onClick={onClick}
      className={`relative flex w-full items-center gap-3 overflow-hidden rounded-xl px-4 py-3 text-sm font-semibold transition-colors ${
        active
          ? "bg-primary text-white shadow-sm before:absolute before:inset-y-0 before:left-0 before:w-1.5 before:bg-secondary-container"
